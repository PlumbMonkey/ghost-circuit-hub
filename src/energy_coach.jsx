  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        <span style={{ fontSize: 10, color: C.dim, letterSpacing: 1 }}>{label}</span>
    ...
    <div style={{ background: C.bg, minHeight: "100vh",
      ...
      <div style={{ background: C.panel, borderBottom: `3px solid ${C.teal}`,
        ...
        <div>
          ...
          <div style={{ fontSize: 9, color: C.mid, letterSpacing: 2 }}>DAILY HABIT + ENERGY TRACKER</div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <SyncBadge status={syncStatus} />
          </div>
          <div style={{ fontSize: 9, color: C.dim }}>{dateKey}</div>
      ...
      {tab === "today" && (
        <div style={{ padding: "16px 20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <span style={{ fontSize: 9, color: C.dim, letterSpacing: 2 }}>DATE</span>
            <input type="date" value={dateKey} onChange={e => setDateKey(e.target.value)}
              style={{ background: C.card2, border: `1px solid ${C.border}`, borderRadius: 4,
                color: C.white, fontFamily: "inherit", fontSize: 11, padding: "4px 8px" }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
            ...
            <div style={{ background: C.card, borderRadius: 10, padding: "14px 16px",
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 10, color: C.dim, letterSpacing: 1, marginBottom: 8 }}>EXERCISE TODAY</div>
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 10, color: C.dim, letterSpacing: 1 }}>WATER GLASSES</span>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {Array.from({ length: 8 }, (_, i) => (
                    <button key={i} onClick={() => setWater(i + 1)} style={{
                      ...
                      color: i < water ? C.blue : C.dim, fontSize: 9, fontFamily: "inherit",
                    }}>💧</button>
                  ))}
                </div>
              </div>
          ...
          <div style={{ background: C.card, borderRadius: 10, padding: "14px 16px",
            ...
            <div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 8 }}>NOTES</div>
            <textarea value={notes} onChange={e => setNotes(e.target.value)}
              ...
              style={{ width: "100%", minHeight: 80, background: C.card2,
                ...
                fontFamily: "inherit", fontSize: 11, padding: "8px 10px",
              ...
              return (
                <div key={dk} onClick={() => { setDateKey(dk); setTab("today"); }}
                  style={{ display: "grid", gridTemplateColumns: "90px 1fr 1fr 1fr 40px 50px",
                    ...
                    cursor: "pointer" }}>
                  <span style={{ fontSize: 10, color: isToday ? C.teal : C.mid, fontWeight: isToday ? 700 : 400 }}>
                    {isToday ? "TODAY" : dk.slice(5)}
                  </span>
                  {d ? (
                    <>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <span style={{ fontSize: 8, color: C.dim, width: 30 }}>NRG</span>
                        <MiniBar value={d.energy} color={colorForScore(d.energy)} />
                        <span style={{ fontSize: 10, color: colorForScore(d.energy), minWidth: 16, textAlign: "right" }}>{d.energy}</span>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <span style={{ fontSize: 8, color: C.dim, width: 30 }}>ZZZ</span>
                        <MiniBar value={d.sleep} color={colorForScore(d.sleep)} />
                        <span style={{ fontSize: 10, color: colorForScore(d.sleep), minWidth: 16, textAlign: "right" }}>{d.sleep}</span>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <span style={{ fontSize: 8, color: C.dim, width: 30 }}>MOD</span>
                        <MiniBar value={d.mood} color={colorForScore(d.mood)} />
                        <span style={{ fontSize: 10, color: colorForScore(d.mood), minWidth: 16, textAlign: "right" }}>{d.mood}</span>
                      </div>
                      <span style={{ fontSize: 12, textAlign: "center" }}>{d.exercise ? "✓" : "·"}</span>
                      <span style={{ fontSize: 10, color: C.blue, textAlign: "right" }}>{d.water ?? 0}💧</span>
                    </>
                  ) : (
                    <span style={{ fontSize: 10, color: C.dim, gridColumn: "2 / -1" }}>— no entry —</span>
                  )}
                </div>
              );
            })}
        ...
        <div style={{ padding: "16px 20px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 16 }}>
            {[
              ...
              <div key={s.label} style={{ background: C.card, borderRadius: 10, padding: 14,
                ...
                <div style={{ fontSize: 8, color: C.dim, letterSpacing: 1, marginBottom: 4 }}>{s.label}</div>
                <div style={{ fontSize: 28, fontWeight: 900, color: s.color }}>{s.val}</div>
                <div style={{ fontSize: 8, color: C.dim }}>last {DAYS_BACK} days</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: C.card, borderRadius: 10, padding: "14px 16px",
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 4 }}>EXERCISE DAYS</div>
                <div style={{ fontSize: 28, fontWeight: 900, color: C.green }}>{exerciseDays}</div>
                <div style={{ fontSize: 9, color: C.dim }}>of last {DAYS_BACK} days logged</div>
              </div>
          ...
          {[
            ...
            return (
              <div key={key} style={{ background: C.card, borderRadius: 10, padding: "12px 16px",
                ...
                <div>
                  <div style={{ fontSize: 9, color: C.dim, letterSpacing: 2 }}>{label} TREND</div>
                  <div style={{ fontSize: 22, fontWeight: 900, color, marginTop: 2 }}>{avgOf(key)}</div>
                  <div style={{ fontSize: 8, color: C.dim }}>avg</div>
                </div>
      ...
      <div style={{ padding: "10px 20px", borderTop: `1px solid ${C.border}`, marginTop: 8,
        fontSize: 9, color: C.dim, display: "flex", justifyContent: "space-between",
        letterSpacing: 1 }}>
